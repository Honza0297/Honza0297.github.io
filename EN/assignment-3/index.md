...
## Assignment 3: Connecting to Wi-Fi
...
- Now both the Wi-Fi controller and the connection itself are set up, so we can turn on Wi-Fi:
...
The following parts are not necessary, but they are useful.
...
NVS will be discussed in the next assignment, so for now it will be a bit of a *blackbox*. We will add this code to `app_main`.
